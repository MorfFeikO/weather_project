...
def get_filepath(data_folder="data"):
    ...
    print(os.path.join(BASEDIR, data_folder))
...
def save_data_to_file2(data):
    ...
    filepath = get_filepath()
    print('filepath', filepath)
    print('saving here -', os.getcwd())
    if not os.path.exists(filepath):
        print('filepath not exists')
        os.mkdir(filepath)
    country, city, created_date, data = data_to_json(data)
    filename = f"{country}_{city}_{created_date}.txt"
    with open(os.path.join(os.path.join(os.getcwd(), "app_data/data"), filename), 'w', encoding='utf-8') as json_file:
        print(data)
        print(json_file)
        json.dump(data, json_file)
